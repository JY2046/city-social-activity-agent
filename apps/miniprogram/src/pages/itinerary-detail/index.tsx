...
export default function ItineraryDetailPage() {
  ...
  }, [activityId, activityReadAdapter, juZhangAdapter, userActivityReadAdapter]);

  function handleBackToList() {
    void navigateBack({
      fail() {
        void switchTab({ url: "/pages/itinerary/index" });
      },
    });
  }
  ...
  return (
    <View className="flow-page">
      <Text className="top-back" onClick={handleBackToList}>
        返回行程列表
      </Text>
      ...
      {stageState?.showFeedback ? <View className="flow-card">
        ...
        <Button
          className="outline-button"
          onClick={() => void navigateTo({ url: `/pages/feedback/index?activityId=${encodeURIComponent(activityId)}` })}
        >
          去反馈
        </Button>
      </View> : null}
      ...
      {shouldShowActivityDetailReturn ? <View className="bottom-link-row">
        <Text
          ...
          onClick={() => void navigateTo({ url: buildActivityDetailUrl(activityId) })}
        >
          返回活动详情页
        </Text>
      </View> : null}
